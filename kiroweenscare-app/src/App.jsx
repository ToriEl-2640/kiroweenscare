...
function App() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-yellow-100 via-pink-100 to-purple-200 relative overflow-hidden flex flex-col items-center">
      {/* Fun floating shapes */}
      ...
      <header className="text-center py-12 relative z-10 w-full max-w-6xl px-4">
        <div className="bg-white rounded-3xl shadow-2xl p-8 border-8 border-purple-400">
          ...
          <p className="text-2xl md:text-3xl text-pink-600 font-bold" style={{fontFamily: 'Bubblegum Sans, sans-serif'}}>
            Where Bugs Become Monsters! 🎃
          </p>
        </div>
      </header>

      <main className="w-full max-w-6xl px-4 pb-16 relative z-10">
        <CodeAnalyzer onAnalyze={setAnalysis} />
        {analysis && <HauntedReport analysis={analysis} />}
        <Guide />
      </main>

      <footer className="text-center py-6 relative z-10 w-full max-w-6xl px-4">
        <div className="bg-white rounded-3xl shadow-2xl p-6 border-8 border-blue-400">
          <p className="text-base md:text-lg text-purple-500 font-bold" style={{fontFamily: 'Fredoka, sans-serif'}}>
            Built with Kiro AI ✨
          </p>
        </div>
      </footer>
